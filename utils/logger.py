...
ignore_keys = ['device', 'save_interval', 'normalize', 'train_transform', 'test_transform',
                   'lrsch', 'weight_decay', 'update', 'build_all', 'parse_task', 'build_backbone', 'task',
                   'schedule', 'label_dict', 'logger', 'train_loader', 'test_loader', 'val_loader', 'train_loader_list',
                   'test_loader_list',  'trainer', 'tester', ]
# 'backbone','clsnet',
class Logger(object):
    """
    My specific logger.
    ...
    def init_backup(self, configs, task):
        print('Backup')
        self.auto_backup('./')
        self.backup_files([os.path.join('./configs',str(task)+'.py')])
    ...
    def save(self, backbone, clsnet, optimizer, global_step=None):
        '''
    ...
    def save_rnn(self, backbone, clsnet, optimizer, global_step=None):
        '''
        save the model/optimizer checkpoints with global step
        param net: (nn.Module) network
        param optimizer: (nn.optim.optimizer)
        ...
        state_net = {
            'backbone': backbone.state_dict(),
            'clsnet_cls': clsnet['cls'].state_dict(),
            'clsnet_rnn': clsnet['RNN'].state_dict(),
        }
        state_optim = {
            'opt': optimizer.state_dict(),
            'epoch': global_step,
    ...
    def load(self, backbone, clsnet, optimizer, global_step=None):
        '''
        Load network and optimizing state given global step.
        '''
        if backbone is not None:
            backbone = self.load_backbone(backbone, global_step)
        if clsnet is not None:
            clsnet = self.load_clsnet(clsnet, global_step)
        if optimizer is not None:
            optimizer = self.load_optim(optimizer, global_step)
        return backbone, clsnet, optimizer

    def load_backbone(self, backbone, global_step=None, path=None):
        """
        ...
        if global_step > 0:
            ...
            self.log_string('==> Resuming net of epoch {}'.format(ckpt_path))
            kwargs = {'map_location': lambda storage, loc: storage}
            ckpt_net = torch.load(
                ckpt_path,
                **kwargs
            )
            ckpt_net['backbone'] = {k.replace('module.', ''): v for k, v in ckpt_net['backbone'].items()}
            msg = backbone.load_state_dict(ckpt_net['backbone'], strict=False)
            if msg.missing_keys is not None:
                print(set(msg.missing_keys))
            return backbone
        else:
            print("Warning: not implemented backbone loading, return default things\n")
            return backbone

    def load_backbone_fromold(self, net, global_step=None):
        """
        Load network. This is a sub-function of self.load()
        """
        if global_step > 0:
            ...
            ckpt_net['net'] = {k.replace('backbone.', ''): v for k, v in ckpt_net['net'].items()}
            msg = net.load_state_dict(ckpt_net['net'], strict=False)
            if msg.missing_keys is not None:
                print(set(msg.missing_keys))
            return net
        else:
    ...
    def load_clsnet_fromold(self, net, global_step=None):
        """
        Load network. This is a sub-function of self.load()
        """
        if global_step > 0:
            ...
            ckpt_net['net'] = {k.replace('', ''): v for k, v in ckpt_net['net'].items()}
            msg = net.load_state_dict(ckpt_net['net'], strict=False)
            if msg.missing_keys is not None:
                print(set(msg.missing_keys))
            return net
    ...
    def load_clsnet(self, clsnet, global_step=None):
        ...
        if global_step > 0:
            ...
            ckpt_net['clsnet'] = {k.replace('module.', ''): v for k, v in ckpt_net['clsnet'].items()}
            msg = clsnet.load_state_dict(ckpt_net['clsnet'], strict=False)
            if msg.missing_keys is not None:
                print(set(msg.missing_keys))
            return clsnet
        else:
            print("Warning: not implemented clsnet loading, return default things\n")
            return clsnet

    def load_BB_and_freeze(self, net, global_step=None, load_path=None):
        """
        Load  and freeze bakcbone. This is a sub-function of self.load()
        """
        ...
        if global_step > 0:
            # self.global_step = global_step
            load_dir = os.path.join(load_path, 'ckp')
            ckpt_path = os.path.join(load_dir, 'net.ckpt{}.pth'.format(global_step))
            self.log_string('==> Resuming&Freeze net of epoch {}'.format(
                ckpt_path
            )
            )
            kwargs = {'map_location': lambda storage, loc: storage}
            ...
            ckpt_net['backbone'] = {k.replace('module.', ''): v for k, v in ckpt_net['backbone'].items()}
            msg = net.load_state_dict(ckpt_net['backbone'], strict=False)
            print(set(msg.missing_keys))
            ...
            print("Warning:Freeze! not implemented network loading, return default things\n")
            return net

    def load_clsnet_and_freeze(self, clsnet, global_step=None, load_path=None, notFreeze=None):
        """
        Load network. This is a sub-function of self.load()
        """
        if notFreeze is None:
            for name, param in clsnet.named_parameters():
                # if name.startswith('backbone'):
                param.requires_grad = False
                print(name)
        if global_step > 0:
    ...
    def load_optim(self, optimizer, global_step=None):
        """
