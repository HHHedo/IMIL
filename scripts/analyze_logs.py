...
"""
Based on mmdetection/tools/analyze_logs.py
What we would do:
    a. We add some annotations.
    b. We make some modification to match my style.
"""
...
def plot_curve(log_dicts, args):
    """
    Notes:
        1. If the metric name is not mAP, 'iter' should be provided. (TODO:
        Make it more general)

    Args:
        log_dicts: (list[dict]) dict struct: {epoch: {acc: 0.9, loss: 0.2..}}
...
def load_txt_logs(txt_logs, key_words=None):
    """
    Load and convert .txt logs to log_dict (log_dict structure is the same
    as in `load_json_logs()`)

    Args:
        txt_logs: (list of str) .txt log files
        key_words: (str) key words to match the file. Find the lines after these key words.

    """
    log_dicts = [dict() for _ in txt_logs]
    for txt_log, log_dict in zip(txt_logs, log_dicts):
        with open(txt_log, 'r') as log_file:
