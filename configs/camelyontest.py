...
class Config(object):
    ...
    def build_data(self):
        ...
        # else:
        #     with open('/remote-home/my/HisMIL/trainset.pickle', 'rb') as f:
        #         self.trainset = pickle.load(f)
        #     self.min_ratios = self.trainset.min_ratios
        #     self.mean_ratios = self.trainset.mean_ratios
        #     self.train_loader = DataLoader(self.trainset, self.batch_size, shuffle=True, num_workers=self.workers)
        # with open('/remote-home/my/HisMIL/testset.pickle', 'rb') as f:
        self.testset = CAMELYON16(self.test_transform)
        self.test_loader = DataLoader(self.testset, self.batch_size, shuffle=False, num_workers=self.workers)
            # with open('/remote-home/my/HisMIL/valset.pickle', 'rb') as f:
    ...
    def load_model_and_optimizer(self):
        ...
        self.backbone, self.clsnet, _ = self.logger.load(self.backbone,
                                                                      self.clsnet,
                                                                      None,
                                                                      self.resume)
    ...
    def build_memoryBank(self):
        ...
        if self.config == 'DigestSeg' or self.config == 'DigestSegAMIL':
            ...
            if self.noisy:
                ...
                noisy = [torch.randn([1]) for _ in range(self.trainset.bag_num)]
                # noisy = random.uniform(-1, 1)
                self.trainset.bag_pos_ratios = [((self.trainset.bag_pos_ratios)[i] + noisy[i]).clamp(max=1.0, min=0)
