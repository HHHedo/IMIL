...
class Config(object):
    ...
    def build_all(self):
        ...
        self.train_loader = DataLoader(self.trainset, self.batch_size, shuffle=True, num_workers=0)
        ##build model
        self.backbone = self.build_backbone(self.backbone).to(self.device)
        self.clsnet = GAttentionClsNet(self.backbone, 1, 128, 1).to(self.device)
        ##build optimizer
        ...
        self.backbone, self.clsnet, self.optimizer = self.logger.load(self.backbone,
                                                                      ...
                                                                      self.optimizer,
                                                                      self.resume)
        self.backbone = self.logger.load_net_and_freeze(self.backbone, self.resume)
        ...
        self.tester = BaseTester(self.backbone, self.clsnet, self.testset, self.test_mmbank, self.logger, self.batch_size)
